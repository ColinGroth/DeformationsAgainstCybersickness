...
public class WarpingRendererFeature : ScriptableRendererFeature
{
    private WarpingRenderPass _warpingPass;
    ...
    public override void Create()
    {
        ...
        _warpingPass =
            new WarpingRenderPass(warpMat, 0, "Warping");
        _warpingPass.renderPassEvent = RenderPassEvent.BeforeRenderingPostProcessing;
    }
    ...
    {
        _warpingPass.ConfigureInput(ScriptableRenderPassInput.Color | ScriptableRenderPassInput.Depth);
        _warpingPass.SetTarget(renderer.cameraColorTargetHandle);
    }
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        if (disposing)
        {
            _warpingPass.Dispose();
        }
    }

    internal class WarpingRenderPass : ScriptableRenderPass
    {
        ...
        RTHandle warp_CameraColorTarget;
        RTHandle warpedRT;
        RTHandle motionVectorsRT;
        RTHandle contrastRT;
        int frameCount = 0;
        ...
        public WarpingRenderPass(Material material, int index, string featureName)
        {
            _warpMat = material;
        ...
        public void Dispose()
        {
            warp_CameraColorTarget?.Release();
            warpedRT?.Release();
        ...
        {
            warp_CameraColorTarget = colorHandle;
        }

        public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ConfigureTarget(warp_CameraColorTarget);
        }
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, m_ProfilingSampler))
            {
                if (warpedRT == null)
                {
                    warpedRT = RTHandles.Alloc(new Vector2(1, 1), 2, colorFormat: GraphicsFormat.R32G32B32A32_SFloat,
                        dimension: TextureDimension.Tex2DArray, name: "WarpedTexture");
                    motionVectorsRT = RTHandles.Alloc(new Vector2(1, 1), 2,
                    ...
                    contrastRT = RTHandles.Alloc(new Vector2(1, 1), 2,
                        colorFormat: GraphicsFormat.R32_SFloat, useMipMap: true, autoGenerateMips: true,
                        dimension: TextureDimension.Tex2DArray, filterMode: FilterMode.Bilinear,
                        name: "ContrastMipTexture");

                    quadsPerRow = (int)((warpedRT.rt.width + (quadSize - 1)) / (float)quadSize);
                    quadsPerColumn = (int)((warpedRT.rt.height + (quadSize - 1)) / (float)quadSize);
                    quadSizeIn2 = new Vector2(2.0f * quadSize / warpedRT.rt.width,
                        2.0f * quadSize / warpedRT.rt.height);
                    ...
                    _warpMat.SetTexture("_MotionVectorsMipTexture", motionVectorsRT);
                    _warpMat.SetBuffer("_Positions", verticesBuffer);

                    optimKernelIdx = optimizationShader.FindKernel("ComputeMovement");
                    optimizationShader.SetTexture(optimKernelIdx, "motionVectors", motionVectorsRT);
                    optimizationShader.SetTexture(optimKernelIdx, "contrast", contrastRT);
                ...
                Blitter.BlitCameraTexture(cmd, warp_CameraColorTarget, motionVectorsRT, _motionVectorsMat, m_PassIndex);

                //-- Prepare Contrast
                // Blitter.BlitCameraTexture(cmd, motionVectorsRT, motionVectorsRT, _prepareMotionMat, m_PassIndex);
                Blitter.BlitCameraTexture(cmd, contrastRT, contrastRT, _prepareContrastMat, m_PassIndex);
                
                //-- Reset Grid
                if (frameCount++ == 2 || EyeTracking._currentMovement == EyeTracking.MovementType.Blink || frameCount % 75 == 0) //
                ...
                CoreUtils.SetRenderTarget(cmd, warpedRT);
                ...
                // Blitter.BlitCameraTexture(cmd, warpedRT, warp_CameraColorTarget, _showEyesMat, m_PassIndex);
            }
