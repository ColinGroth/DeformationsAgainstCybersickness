...
public class WaypointMovementNew : MonoBehaviour
{
    ...
    private string filePathEyeData = Application.dataPath + "/../experimentData/";
    ...
    void Update()
    {
        ...
        if (!moving && Input.GetKeyDown(KeyCode.Space))
        {
            if (id != 0)
            {
                ...
                filePathEyeData = Path.Combine(filePathEyeData, fileName + "_Eye.txt");
                if (!File.Exists(filePathEyeData))
                {
                    File.Create(filePathEyeData).Close();
                    UnityEngine.Debug.Log("Created file " + fileName + "_Eye.txt");
                    string oldPath = filePath;
                    filePath = filePathEyeData;
                    string data =
                        "Time in ms, Eytracking current movement, eyePosProjLeftX, eyePosProjLeftY, eyePosProjRightX, eyePosProjRightY";
        ...
        if (moving)
        {
            ...
            Matrix4x4 gpuProjLeft = GL.GetGPUProjectionMatrix(camera.GetStereoProjectionMatrix(Camera.StereoscopicEye.Left), true);
            ...
            Matrix4x4 gpuProjRight = GL.GetGPUProjectionMatrix(
                camera.GetStereoProjectionMatrix(Camera.StereoscopicEye.Right), true);
            ...
            string data = stopwatch.ElapsedMilliseconds + " " + EyeTracking._currentMovement + " " + eyePosProjLeft.x + " " + eyePosProjLeft.y + " " + eyePosProjRight.x + " " + eyePosProjRight.y;
            string oldPath = filePath;
            filePath = filePathEyeData;
            WriteDataToFile(data);
            filePath = oldPath;
